 ...
 */
public class Item2Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    private Context context;
    private DataBean.Data.BestSellers bestSellers;
    private OnRecyclerItemClickListener listener;

    public Item2Adapter(Context context, DataBean.Data.BestSellers bestSellers) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder1) {
            Glide.with(context).load(bestSellers.goodsList.get(position).goods_img).into(((MyViewHolder1) holder).item2_iv);
            ((MyViewHolder1) holder).item2_name.setText(bestSellers.goodsList.get(position).goods_name);
            ((MyViewHolder1) holder).item2_market.setText("￥" + bestSellers.goodsList.get(position).market_price);
            ((MyViewHolder1) holder).item2_shop.setText("￥" + bestSellers.goodsList.get(position).shop_price);
    ...
    public int getItemViewType(int position) {
        if (position==bestSellers.show_number){
            return 2;
        }else{
            return 1;
        }
    ...
    public int getItemCount() {
        return bestSellers.show_number+1;
    }
